...
public class ProductGetShowcases
{
    ...
    public static async Task<IResult> Action(ApplicationDbContext context, int page = 1, int row = 10, string orderBy = "name")
    {
        ...
        if (row > 10)
            return Results.Problem(title: "No máximo 10 linhas", statusCode: 400);
        var queryBase = context.Products.AsNoTracking().Include(c => c.Category).Where(p => p.HasStock && p.Category.Active);
        ...
        if (orderBy == "name")
            ...
            return Results.Problem(title: "Ordernar apenas por name ou price", statusCode: 400);

        var queryFilter = queryBase.Skip((page - 1) * row).Take(row);



        var product = queryFilter.ToList();
